...
tool = {
	...
	copy_other_files : function(){
		var _x = function( url ){
			grunt.file.copy( url , tool.get_pub_file_path( url ) );
		};
	...
	uglify_css : function( replace , dest ){
		...
		for( var i = _files.length; i--; ){
			_url 	= replace.src + _files[ i ];
			if( grunt.file.exists( _url ) ){
				_buffer.push( grunt.file.read( _url ).toString() );
			};
		};
		_code = new mincss( { banner : "/*! " + new Date() + " */" } ).minify( _buffer.join( "" ) );
	...
	replace_js : function( content , replace ){
		var _replace	= replace.key + "$1" + replace.key;
			_al 		= content.replace( /(<script[^\\>]*src=[^\\>]*><\/script>)/gi , _replace ).split( replace.key ),
			_js 		= [],
			_url 		= "js/" + replace.md5 + ".js";
		...
		if( _js.length ){
			replace.js = _js;
			tool.uglify_js( replace , replace.dest + _url );
			tool.concat_done( _js , replace , replace.dest + _url );
		};
		return _al.join( "" );
	},
	/*!
	...
	replace_css: function( content , replace ){
		...
		if( _css.length ){
			replace.css = _css;
			tool.uglify_css( replace , replace.dest + _url );
			tool.concat_done( _css , replace , replace.dest + _url );
		};
		return _al.join( "" );
	},
	/*!
	...
	replace 	: function( fp , dest ){
		var _content 	= grunt.file.read( fp ).toString().
			...
			_replace 	= { 
				...
				dest: dest, 
				key : _tp,
				md5 : tool.md5( "ainghkh" + _tp + parseInt( Math.random() * 999999999 ) )
			};
		_content 	= tool.replace_css( _content , _replace );
		_content 	= tool.replace_js( _content , _replace );
		grunt.file.write( dest + _replace.fp.replace( /[\d|\w]*[\\|\/](.*)/gi , "$1" ) , _content );
	},
	handle_replace_html : function(){
		var _html = config.file_path.html;
		for( var a in _html ){
			tool.replace( a , config.dir.pub_dir );
		};
	...
	get_html_files 	: function( dir ){
		grunt.file.recurse( dir , function( path ){
			if( /.*\.html$/gi.test( path ) ){
				...
				grunt.file.copy( path , tool.get_pub_file_path( path ) );
			};
		} );
	},
